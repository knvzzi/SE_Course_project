...
#pragma endregion 
	Mfst::RC_STEP Mfst::step(Log::LOG& log)
	{
		...
		{
			if (GRB::Rule::Chain::isN(st.top()))// ≈сли вершина стека - нетерминал
			{
				GRB::Rule rule;
	...
	// ћетод push_chain помещает цепочку правила в стек
	...
	// ћетод savestate сохран€ет текущее состо€ние анализатора
	...
	// ћетод resetstate восстанавливает состо€ние анализатора из сохраненного состо€ни€
	...
	// ћетод savediagnosis сохран€ет диагноз ошибки в массив
	...
	// ћетод start выполн€ет синтаксический анализ
	bool Mfst::start(Log::LOG& log)
	{
		bool rc = false;
		RC_STEP rc_step = SURPRISE;
		char buf[MFST_DIAGN_MAXSIZE]{};
		rc_step = step(log); // ¬ыполнение первого шага синтаксического анализа
		while (rc_step == NS_OK || rc_step == NS_NORULECHAIN || rc_step == TS_OK || rc_step == TS_NOK)
			rc_step = step(log); // ¬ыполнение шагов до завершени€ анализа
	...
	// ћетод getCSt формирует строку дл€ текущего состо€ни€ стека
	...
	// ћетод getCLenta формирует строку дл€ текущей части ленты
	...
	// ћетод getDiagnosis формирует строку дл€ диагноза ошибки
	...
	bool Mfst::savededucation()
	{
		MfstState state;
		GRB::Rule rule;
		deducation.nrules = new short[deducation.size = storestate.size()]; // ¬ыделение пам€ти дл€ массива deducation.nrules и установка размера.
		deducation.nrulechains = new short[deducation.size]; // ¬ыделение пам€ти дл€ массива deducation.nrulechains и установка размера.

		for (unsigned short i = 0; i < storestate.size(); i++)
		{
			state = storestate.c[i]; // ѕолучение элемента массива storestate.
			deducation.nrules[i] = state.nrule; // «апись значени€ state.nrule в массив deducation.nrules.
			deducation.nrulechains[i] = state.nrulechain; // «апись значени€ state.nrulechain в массив deducation.nrulechains.
		}
		return true;
	}
}
